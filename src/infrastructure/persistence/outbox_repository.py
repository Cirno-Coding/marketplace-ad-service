...
class SQLAlchemyOutboxRepository(OutboxRepository):
    ...
    async def fetch_unpublished(self, limit: int) -> list[OutboxMessage]:
        stmt = select(OutboxModel).where(OutboxModel.published_at.is_(None)).order_by(OutboxModel.id.asc()).limit(limit).with_for_update(skip_locked=True)
    ...
    async def mark_published(self, ids: list[int]) -> None:
        if not ids:
            return
        await self._session.execute(update(OutboxModel).where(OutboxModel.id.in_(ids)).values(published_at=func.now()))
